...
def get_neighbours(x, y, z, space_, radius=1):
    neighbours = []
    for idx in range(-1, 2):
        for idy in range(-1, 2):
            for idz in range(-1, 2):
                if not (idx==0 and idy==0 and idz==0):
                    try:
                        neighbours.append(space_[x+idx, y+idy, z+idz])
    ...
    for x in range(old_space.shape[0]):
        for y in range(old_space.shape[1]):
            for z in range(old_space.shape[2]):
                neighbours = get_neighbours(x, y, z, old_space)
                if old_space[x, y, z] == '#' and not (2<=neighbours.count('#')<=3):
                    space[x,y,z] = '.'
                elif neighbours.count('#') == 3:
                    space[x, y, z] = '#'

                active_cell_count += 1 if space[x, y, z] == '#' else 0
...
space = np.zeros((cube_size,cube_size,cube_size), dtype='U1') # Make a 10 by 20 by 30 array

# Init space with input
for idx, x in enumerate(input):
    for idy, y in enumerate(x):
        for idz, z in enumerate(y):
            space[idx+zero_offset, idy+zero_offset, idz+zero_offset] = z
